std::unordered_map<std::wstring, int32_t> FProperty::MemberOffsets{};
...
uint64 FProperty::GetPropertyFlags()
{
    ...
    return Helper::Casting::ptr_cast_deref<uint64>(this, offset->second);
}
const uint64 FProperty::GetPropertyFlags() const
{
    static auto offset = MemberOffsets.find(STR("PropertyFlags"));
    if (offset == MemberOffsets.end()) { throw std::runtime_error{"Tried getting member variable 'FProperty::PropertyFlags' that doesn't exist in this engine version."}; }
    return Helper::Casting::ptr_cast_deref<const uint64>(this, offset->second);
}
