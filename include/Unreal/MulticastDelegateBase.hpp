#ifndef UNREAL_MULTICASTDELEGATEBASE_HPP
#define UNREAL_MULTICASTDELEGATEBASE_HPP

#include <Unreal/DelegateBase.hpp>
...
public:
     ...
     */
    inline bool IsBoundToObject(const void* InUserObject) const {
        for (const TDelegateBase<UserPolicy>& DelegateBaseRef : InvocationList) {
    ...
    int32 RemoveAll(const void* InUserObject) {
        ...
        if (InvocationListLockCount > 0) {
            ...
            for (int32 InvocationListIndex = 0; InvocationListIndex < InvocationList.Num();) {
                ...
                if (DelegateInstance == nullptr || DelegateInstance->HasSameObject(InUserObject) || DelegateInstance->IsCompactable()) {
            ...
            CompactionThreshold = std::max(2, 2 * InvocationList.Num());
...
protected:
    ...
    inline TMulticastDelegateBase() : CompactionThreshold(2), InvocationListLockCount(0) {
    }
protected:
    /**
...
#endif //UNREAL_MULTICASTDELEGATEBASE_HPP
