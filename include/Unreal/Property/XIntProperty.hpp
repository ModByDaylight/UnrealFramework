#ifndef RC_UNREAL_XINTPROPERTY_HPP
#define RC_UNREAL_XINTPROPERTY_HPP
...
namespace RC::Unreal
{
    class RC_UE_API XIntProperty : public XNumericProperty
    {
    private:

    public:
        constexpr static inline bool is_trivial = true;
        constexpr static inline int32_t size = sizeof(int32_t);
        ...
        {
            if (!m_static_obj_variant.is_valid()) { throw std::runtime_error{"[XIntProperty::static_class] m_static_obj_variant is not valid"}; }
...
#endif //RC_UNREAL_XINTPROPERTY_HPP
