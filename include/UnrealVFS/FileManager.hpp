#ifndef UNREAL_FILEMANAGER_HPP
#define UNREAL_FILEMANAGER_HPP
...
class RC_UE_API IFileManager {
protected:
    /** Construtor. */
    IFileManager() {}
public:
    /**
     * Updates the active instance of IFileManager to the provided one
     * Internal function, should only be called at startup
     */
    static void SetFileManager(IFileManager* UnderlyingManager);
    ...
    /** Deletes a file. */
    virtual bool Delete(const TCHAR* Filename, bool RequireExists = false, bool EvenReadOnly = false, bool Quiet = false) = 0;

    /** Copies a file. */
    virtual uint32 Copy(const TCHAR* Dest, const TCHAR* Src, bool Replace = true, bool EvenIfReadOnly = false, bool Attributes = false, FCopyProgress* Progress = nullptr, EFileRead ReadFlags = FILEREAD_None, EFileWrite WriteFlags = FILEWRITE_None) = 0; // utility

    /** Moves/renames a file. */
    virtual bool Move(const TCHAR* Dest, const TCHAR* Src, bool Replace = true, bool EvenIfReadOnly = false, bool Attributes = false, bool bDoNotRetryOrError = false) = 0;
    ...
    /** Deletes a directory. */
    virtual bool DeleteDirectory(const TCHAR* Path, bool RequireExists = false, bool Tree = false) = 0;
