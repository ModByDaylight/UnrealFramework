...
RC::Function<return_type(const class_name*)> func = *std::bit_cast<void**>(vtable + it->second); \
if (!func.is_ready()) \
{ \
...
std::byte* vtable = std::bit_cast<std::byte*>(*std::bit_cast<std::byte**>(this)); \
RC::Function<return_type(const class_name*, params)> func = *std::bit_cast<void**>(vtable + it->second); \
if (!func.is_ready()) \
{ \
throw std::runtime_error{"Function '" #function_name "' not available"}; \
}                                                                          \
