#ifndef UNREAL_SHAREDPOINTERINTERNALS_HPP
#define UNREAL_SHAREDPOINTERINTERNALS_HPP
...
namespace SharedPointerInternals {
    ...
    public:
        explicit TReferenceControllerWithDeleter(ObjectType* InObject, DeleterType&& Deleter) : DeleterType(MoveTemp(Deleter)), Object(InObject) {
        }
    ...
    inline FReferenceControllerBase* NewCustomReferenceController(ObjectType* Object, DeleterType&& Deleter) {
        return new TReferenceControllerWithDeleter<ObjectType, std::remove_reference_t<DeleterType>>(Object, Forward<DeleterType>(Deleter));
    }
    ...
    struct FReferenceControllerOps<ESPMode::ThreadSafe> {
        ...
        static FORCEINLINE const int32 GetSharedReferenceCount(const FReferenceControllerBase* ReferenceController) {
            ...
            return _InterlockedCompareExchange((volatile long*) &ReferenceController->SharedReferenceCount, 0, 0);
        }
        ...
        static FORCEINLINE void AddSharedReference(FReferenceControllerBase* ReferenceController) {
            _InterlockedIncrement((volatile long*) &ReferenceController->SharedReferenceCount);
        }
        ...
        static bool ConditionallyAddSharedReference(FReferenceControllerBase* ReferenceController) {
            for( ; ; )
            {
                ...
                // multiple threads.
                const int32 OriginalCount = _InterlockedCompareExchange((volatile long*) &ReferenceController->SharedReferenceCount, 0, 0);
                if( OriginalCount == 0 )
                {
                ...
                const int32 ActualOriginalCount = _InterlockedCompareExchange((volatile long*) &ReferenceController->SharedReferenceCount, OriginalCount + 1, OriginalCount);
        ...
        static FORCEINLINE void ReleaseSharedReference(FReferenceControllerBase* ReferenceController) {
            if(_InterlockedDecrement((volatile long*) &ReferenceController->SharedReferenceCount ) == 0) {
        ...
        static FORCEINLINE void AddWeakReference(FReferenceControllerBase* ReferenceController) {
            _InterlockedIncrement((volatile long*) &ReferenceController->WeakReferenceCount);
        }

        /** Releases a weak reference to this counter */
        static void ReleaseWeakReference(FReferenceControllerBase* ReferenceController) {
            if(_InterlockedDecrement((volatile long*) &ReferenceController->WeakReferenceCount) == 0) {
...
#endif //UNREAL_SHAREDPOINTERINTERNALS_HPP
