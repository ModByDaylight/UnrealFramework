#ifndef RC_UNREAL_XFLOATPROPERTY_HPP
#define RC_UNREAL_XFLOATPROPERTY_HPP
...
namespace RC::Unreal
{
    class RC_UE_API XFloatProperty : public XNumericProperty
    {
    private:

    public:
        constexpr static inline bool is_trivial = true;
        constexpr static inline int32_t size = sizeof(float);
        using InnerType = float;
        ...
        {
            if (!m_static_obj_variant.is_valid()) { throw std::runtime_error{"[XFloatProperty::static_class] m_static_obj_variant is not valid"}; }
...
#endif //RC_UNREAL_XFLOATPROPERTY_HPP
