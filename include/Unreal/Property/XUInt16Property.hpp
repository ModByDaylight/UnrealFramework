#ifndef RC_UNREAL_XUINT8PROPERTY_HPP
#define RC_UNREAL_XUINT8PROPERTY_HPP
...
namespace RC::Unreal
{
    class RC_UE_API XUInt16Property : public XNumericProperty
    {
    private:

    public:
        constexpr static inline bool is_trivial = true;
        constexpr static inline int32_t size = sizeof(uint16_t);
        using InnerType = uint16_t;
        ...
        {
            if (!m_static_obj_variant.is_valid()) { throw std::runtime_error{"[XUInt16Property::static_class] m_static_obj_variant is not valid"}; }
...
#endif //RC_UNREAL_XUINT8PROPERTY_HPP
