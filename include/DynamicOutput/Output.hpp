...
namespace RC::Output
{
    ...
    public:
        ...
        auto send(File::StringViewType content, OptionalArg optional_arg) -> void
        {
            ...
            for (const auto& device : m_opened_devices)
            {
                ...
                {
                    device->receive_with_optional_arg(std::format(content), static_cast<int32_t>(optional_arg));
                }
                else
                {
                    device->receive(std::format(content));
                }
            }
        }
        ...
        auto send(File::StringViewType content, FmtArgs... fmt_args) -> void
        {
            ...
            for (const auto& device : m_opened_devices)
            {
                ...
                if (device->has_optional_arg())
                {
                    device->receive_with_optional_arg(std::format(content, fmt_args...), 0);
                }
                else
                {
                    device->receive(std::format(content, fmt_args...));
                }
            }
        }
        ...
        auto send(File::StringViewType content, OptionalArg optional_arg, FmtArgs... fmt_args) -> void
        {
            ...
            for (const auto& device : m_opened_devices)
            {
                ...
                if (device->has_optional_arg())
                {
                    device->receive_with_optional_arg(std::format(content, fmt_args...), static_cast<int32_t>(optional_arg));
                }
                else
                {
                    device->receive(std::format(content, fmt_args...));
        ...
        auto send(File::StringViewType content, FmtArg fmt_arg, FmtArgs... fmt_args) -> void
        {
            ...
            for (const auto& device : m_opened_devices)
            {
                ...
                if (device->has_optional_arg())
                {
                    device->receive_with_optional_arg(std::format(content, fmt_arg, fmt_args...), optional_arg);
                }
                else
                {
                    device->receive(std::format(content, fmt_args...));
    ...
    auto send(File::StringViewType content, FmtArgs... fmt_args) -> void
    {
        for (const auto& device : DefaultTargets::get_default_devices_ref())
        {
            ...
            if (device->has_optional_arg())
            {
                device->receive_with_optional_arg(std::format(content, fmt_args...), optional_arg);
            }
            else
            {
                device->receive(std::format(content, fmt_args...));
