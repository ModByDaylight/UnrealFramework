...
namespace RC::File
{
    ...
    auto WinFile::close_current_file() -> void
    {
        ...
        {
            THROW_INTERNAL_FILE_ERROR(std::format("[WinFile::close_current_file] Was unable to close file, error: {}", GetLastError()))
        }
    }
    ...
    auto WinFile::create_all_directories(const std::filesystem::path& file_name_and_path) -> void
    {
        ...
        {
            THROW_INTERNAL_FILE_ERROR(std::format("[WinFile::create_all_directories] Tried creating directories '{}' but encountered an error. Error: {}", e.what()))
        }
    }
    ...
    auto WinFile::read_all() const -> StringType
    {
        ...
        else
        {
            ...
            file_contents.resize(stream.tellg());
    ...
    auto WinFile::open_file(const std::filesystem::path& file_name_and_path, const OpenProperties& open_properties) -> WinFile
    {
        ...
        if constexpr (sizeof(CharType) > 1)
        {
            file.set_file(CreateFileW(file_name_and_path.wstring().c_str(), desired_access, FILE_SHARE_READ, nullptr, creation_disposition, FILE_ATTRIBUTE_NORMAL, nullptr));
        }
        else
        {
            file.set_file(CreateFileA(file_name_and_path.string().c_str(), desired_access, FILE_SHARE_READ, nullptr, creation_disposition, FILE_ATTRIBUTE_NORMAL, nullptr));
        }
