...
namespace RC::Helper::Casting
{
    ...
    template<typename To, typename From>
    auto ptr_cast_deref_safe(From ptr, int32_t offset) -> To
    {
        ...
        if (!ReadProcessMemory(GetCurrentProcess(), data_ptr, &is_valid_ptr_buffer, 0x8, &bytes_read))
        {
            return 0;
        }
        else
    ...
    // Compatibility with older code from before 'ptr_cast' existed
    template<typename To, typename From>
    static auto offset_deref_safe(From* base_ptr, int32_t offset) -> To
    {
        return ptr_cast_deref_safe<To>(base_ptr, offset);
    }
}
